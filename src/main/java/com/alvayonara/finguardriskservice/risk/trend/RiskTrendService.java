...
public class RiskTrendService {
    @Autowired
    private RiskTrendRepository riskTrendRepository;

    public Mono<RiskTrendResponse> getTrend(Long userId, int days) {
        LocalDateTime since = LocalDateTime.now().minusDays(days);
        return riskTrendRepository.findSince(userId, since)
                .collectList()
                .map(list -> {
                    ...
                    List<RiskTrendPoint> points = latestPerDay.values().stream()
                            .sorted(Comparator.comparing(RiskLevelHistory::getOccurredAt))
                            .map(riskLevelHistory -> RiskTrendPoint.builder()
