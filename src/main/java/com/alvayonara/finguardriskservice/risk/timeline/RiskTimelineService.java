...
public class RiskTimelineService {
    @Autowired
    private RiskTimelineRepository riskTimelineRepository;

    public Mono<RiskTimelineResponse> getTimeline(Long userId, String cursorTime, Long cursorId, int limit) {
        Flux<RiskLevelHistory> source;
        if (Objects.isNull(cursorTime)) {
            source = riskTimelineRepository.findFirstPage(userId, limit);
        } else {
            source = riskTimelineRepository.findNextPage(
                    userId,
        ...
        return source.collectList().map(list -> {
            List<RiskTimelineItem> items = list.stream().map(h -> RiskTimelineItem.builder()
                            .level(h.getNewLevel())
