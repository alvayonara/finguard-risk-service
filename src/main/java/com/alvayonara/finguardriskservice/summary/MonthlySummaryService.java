...
public class MonthlySummaryService {
    ...
    public Mono<Void> applyTransaction(TransactionCreatedEvent event) {
        ...
        return monthlySummaryRepository.findByUserIdAndMonthKey(event.getUserId(), monthKey)
                .flatMap(existing -> updateExisting(existing, event))
                .switchIfEmpty(insertNew(event.getUserId(), monthKey, event))
                .then();
    }

    private Mono<MonthlySummary> updateExisting(MonthlySummary summary, TransactionCreatedEvent event) {
        applyAmount(summary, event);
        summary.setUpdatedAt(LocalDateTime.now());
        return monthlySummaryRepository.save(summary);
    }

    private Mono<MonthlySummary> insertNew(Long userId, String monthKey, TransactionCreatedEvent event) {
