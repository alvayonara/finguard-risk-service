...
public class AppConfigController {
  ...
  public AppConfigController(
      FeatureFlagService featureFlagService, SubscriptionService subscriptionService) {
    this.featureFlagService = featureFlagService;
    this.subscriptionService = subscriptionService;
  }

  @GetMapping
  public Mono<AppConfigResponse> getConfig(
    ...
    return subscriptionService
        .resolveEffectivePlan(userUid)
        .flatMap(
            plan -> {
              FeatureContext context = new FeatureContext(userUid, plan, platform, version);
              Mono<Boolean> subscription =
                  featureFlagService.isEnabled(FeatureFlagConstants.SUBSCRIPTION_ENABLED, context);
              Mono<Boolean> community =
                  featureFlagService.isEnabled(FeatureFlagConstants.COMMUNITY_ENABLED, context);
