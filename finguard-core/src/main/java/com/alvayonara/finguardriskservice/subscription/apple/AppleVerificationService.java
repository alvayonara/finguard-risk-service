...
public class AppleVerificationService {
        ...
        return verifier.verify(signedPayload)
                .flatMap(outerJwt -> {
                    try {
                        ...
                        LocalDateTime signedAt = LocalDateTime.ofInstant(
                                        outerClaims.getIssueTime().toInstant(),
                                        ZoneOffset.UTC
                                );
                        ...
                        return verifySignedTransaction(signedTransactionInfo)
                                .map(result -> new AppleWebhookNotification(
                                        eventId,
                                        type,
                        ...
                        return Mono.error(
                                new RuntimeException("Invalid Apple outer JWT", e)
                        );
                    }
                });
    }
    ...
    private SubscriptionValidationResult mapToValidationResult(SignedJWT jwt) {
        try {
            ...
            boolean autoRenew = claims.getBooleanClaim("autoRenewStatus") != null
