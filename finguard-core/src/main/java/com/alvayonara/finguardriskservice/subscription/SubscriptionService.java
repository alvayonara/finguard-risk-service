...
public class SubscriptionService {
  ...
  public Mono<Void> purchaseSubscription(String userUid, SubscriptionPurchaseRequest request) {
    ...
    switch (platform) {
      case ANDROID -> validationMono =
          googlePlayVerificationService.verify(request.productId(), request.transactionData());
      case IOS -> validationMono =
          appleVerificationService.verifySignedTransaction(request.transactionData());
      default -> {
  ...
  private Mono<String> revalidateSubscription(Subscription subscription) {
    ...
    if (SubscriptionPlatform.ANDROID.name().equals(subscription.getPlatform())) {
      validationMono =
          googlePlayVerificationService.verifyLatestByToken(
              subscription.getExternalTransactionId());
    } else {
      validationMono =
          appleVerificationService.verifyLatestByTransaction(
              subscription.getExternalTransactionId());
    }
    return validationMono
        .flatMap(
    ...
    return subscriptionRepository
        ...
        .flatMap(
            existing -> {
              ...
              existing.setStatus(
                  canceled ? SubscriptionStatus.CANCELED.name() : SubscriptionStatus.ACTIVE.name());
  ...
  public Mono<Void> syncAppleSubscription(
      String externalTransactionId, SubscriptionValidationResult result) {
